...
import { Bell, CheckCheck, Trash2, Calendar, BellOff } from 'lucide-react'
...
export default function NotificationsPage() {
  ...
  const clearAll = async () => {
    if (!user?.id || !API_URL || !confirm("Clear all alerts permanently?")) return

    try {
      await fetch(`${API_URL}/api/notifications/${user.id}`, {
        method: 'DELETE'
      })
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 space-y-8 min-h-screen">
      <div className="flex justify-between items-end border-b pb-6">
        ...
        {notifications.length > 0 && (
          <div className="flex gap-4">
            ...
            <button
              onClick={clearAll}
              className="text-xs font-bold text-red-500 flex items-center gap-1 uppercase tracking-tighter"
            >
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="py-20 text-center font-serif italic text-slate-400">
            Loading alerts...
          </div>
        ) : notifications.length === 0 ? (
            ...
            <div
              ...
              className={cn(
                ...
                n.isRead
                  ...
                  : "bg-white border-primary/20 shadow-sm shadow-primary/5 hover:border-primary/40"
              )}
            >
