...
import PaymentModal from '@/components/PaymentModal' // <--- IMPORT YOUR NEW MODAL
import { 
  Calendar, 
  Clock, 
  Loader2, 
...
export default function ClientDashboard() {
  ...
  // --- STK PUSH HANDLER ---
  const triggerStkPush = async (phone: string) => {
    if (!selectedBooking) return;
    
    setPaymentLoading(selectedBooking.id)
    try {
      const res = await fetch('http://localhost:5000/api/payments/stk-push', {
        ...
        body: JSON.stringify({
          ...
          vendorName: "Splendour"
        })
      });

      if (res.ok) {
        toast.success("STK Push sent! Please check your phone.")
        // Redirect to success page after a short delay to simulate M-Pesa processing
        setTimeout(() => {
          window.location.href = '/client/payment-success'
        }, 3000)
      } else {
        const data = await res.json();
        toast.error(data.details?.errors?.[0]?.detail || "M-Pesa service busy. Try again.")
      }
    } catch (err) {
  ...
  return (
    <DashboardShell userType="client" userName={user.name}>
      ...
      <div className="grid gap-6 md:grid-cols-3 mb-8">
        <Card className="bg-slate-900 text-black shadow-xl border-none">
        ...
        <Card className="border-primary/20 bg-primary/5">
          <CardHeader className="pb-2 text-primary font-bold tracking-widest text-xs uppercase">
          ...
          <CardContent>
            ...
            <p className="text-[10px] mt-1 text-primary/70 italic uppercase tracking-tighter">Pending or Unpaid</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="pb-2 text-muted-foreground font-bold text-xs uppercase tracking-widest">
             History
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold">{bookings.length}</div>
            <p className="text-[10px] mt-1 text-muted-foreground italic uppercase tracking-tighter">Total Transactions</p>
          </CardContent>
        </Card>
      </div>
              ...
              {bookings.map((booking) => (
                <div key={booking.id} className="p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 hover:bg-muted/5 transition-all">
                  <div className="space-y-1 flex-1">
                    <div className="flex items-center gap-3">
                      ...
                      <Badge className={cn(
                        ...
                        booking.status === 'ACCEPTED' && "bg-blue-100 text-blue-700 border-none",
                  ...
                  <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 pt-4 md:pt-0">
                    ...
                    {booking.status === 'ACCEPTED' && (
                      <Button 
                        ...
                        className={cn(
                          ...
                          "border-2 border-emerald-700 shadow-[4px_4px_0px_0px_rgba(5,150,105,1)]",
                      ...
                      >
                        {paymentLoading === booking.id ? (
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="tracking-tighter uppercase">Pay Now</span>
                          </div>
                        )}
                      </Button>
                    )}
                    ...
                    {booking.status === 'PAID' && (
                      <div className="flex items-center gap-2 text-emerald-600 font-bold uppercase text-[10px] bg-emerald-50 px-3 py-2 rounded-lg border border-emerald-100">
