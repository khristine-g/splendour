  ...
  return (
    <div className="p-6 md:p-10 space-y-10 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <QuickStat
          title="Total Revenue"
          value={`KSh ${stats.revenue?.toLocaleString?.() || 0}`}
      ...
      <section className="space-y-6">
        <div className="flex items-center gap-2 text-slate-400">
          ...
          <h2 className="text-xl font-serif font-bold italic text-slate-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {vendors.map((vendor) => (
            <Card
              key={vendor.id}
              className="border-none shadow-sm bg-white overflow-hidden group border-t-4 border-t-primary/20"
            >
                ...
                <div className="p-5 flex items-center justify-between border-b border-slate-50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <Badge variant="secondary" className="text-[9px] mt-1 h-4 uppercase">
                        {vendor.category}
                      </Badge>
                    </div>
                  </div>
                ...
                <div className="p-4 space-y-2 max-h-48 overflow-y-auto bg-slate-50/30">
                  ...
                  {vendor.services?.length > 0 ? (
                    vendor.services.map((svc: any) => (
                      <div
                        key={svc.id}
                        className="flex items-center justify-between bg-white p-2 rounded-md border border-slate-100 group/item"
                      >
                        ...
                        <button
                          onClick={() =>
                            handleDeleteService(svc.id, svc.title)
                          }
      ...
      <CardContent className="pt-4 space-y-3">
        {items.length > 0 ? (
          items.map((item: any) => (
            <div
              key={item.id}
              className="p-3 bg-slate-50/50 rounded-lg border border-slate-100 text-sm"
            >
              {isBooking ? (
                <p className="text-slate-700">
                  <b>{item.client?.name}</b> booked{' '}
                  <b>{item.service?.title}</b>
                </p>
              ) : (
